...
(function (undefined) {
    ...
    var Kalendae = function (targetElement, options) {
        ...
        self._mylist = [];
        if (options.selected) {
            var n = options.selected[0]._d.getFullYear() == options.selected[1]._d.getFullYear() && (options.selected[0]._d.getMonth() == options.selected[1]._d.getMonth()) && (options.selected[0]._d.getDate() == options.selected[1]._d.getDate());
            if (!n) {
                self._mylist = options.selected.slice(0);
            }
        }
        if (!!opts.selected)
                ...
                if (self.publish('date-clicked', self, [clickedDate]) !== false) {
                    ...
                    switch (opts.mode) {
                        ...
                        case 'week':
                            ...
                            break;
                        case 'month':
                            self.monthSelected(clickedDate);
                            break;
                        case 'single':
                            /* falls through */
                        default:
	...
	classes : {
		...
		caption         :'k-caption',//
        spanConfirm     :'k-span-confirm',
        selectedStartTime:'k-selected-start-time',
        spanCancel      :'k-span-cancel',
		header          :'k-header',//
		days            :'k-days',
		week            :'k-week',
        month           :'k-month',
		...
		daySelectedFirst :'k-selected-first',
		daySelectedLast :'k-selected-last',
		dayInRange      :'k-range',
		dayToday        :'k-today',
		monthSeparator  :'k-separator',//
        straightLine    :'k-straight-line',//
        selectedDate    :'k-selected-date',
        startTime       :'k-start-time',//
        endTime         :'k-end-time',//
        confirmBtn      :'k-confirm-btn',//
        cancelBtn       :'k-cancel-btn',//
        disableConfirmBtn:'k-disable-confirm-btn',
        disableCancelBtn:'k-disable-cancel-btn',
        disableEndTime:'k-disable-end-time',
...
//				if (!a || !b) return 0;
        ...
        ascending: function (firstDay, secondDay) {
            return firstDay._i.substring(0, 4) > secondDay._i.substring(0, 4) || (firstDay._i.substring(0, 4) == secondDay._i.substring(0, 4) && firstDay._i.substring(5, 7) > secondDay._i.substring(5, 7)) || (firstDay._i.substring(0, 4) == secondDay._i.substring(0, 4) && firstDay._i.substring(5, 7) == secondDay._i.substring(5, 7) && firstDay._i.substring(8, 10) > secondDay._i.substring(8, 10)) ? 1 : -1;
        },
        addSelected: function (date, draw) {
            ...
            switch (this.settings.mode) {
                case 'multiple':
                    if (!this.isSelected(date) && this._sel.length < 8)
                        this._sel.push(date);
                    else
                        return false;
                    if (!this._mylist.length) {
                        var list = this._sel.slice(0);
                        list.sort(this.ascending);
                        var comma = list.length > 1 ? "," : "";
                        var dateList = "";
                        for (var i = 0; i < list.length; i++) {
                            if (list.length - 1 == i) {
                                comma = "";
                            }
                            dateList += list[i]._i + comma;
                        }
                        startSelectD.text(dateList);
                        endSelectD.text("");
                    }
                    break;
                case 'range':
                    if (this._sel.length !== 1) {
                        this._sel = [date];
                        startSelectD.text(date._i);
                        endSelectD.text(date._i);
                    }
                    else {
                        ...
                        var firstMonth;
                        var firstDay;
                        firstDate = startSelectD.text();
                        if (firstDate) {
                            firstYear = firstDate.substring(0, 4);
                            firstMonth = firstDate.substring(5, 7);
                            firstDay = firstDate.substring(8, 10);
                        }
                        if (date._i) {
                            if (firstYear > date._i.substring(0, 4)) {//判断年份
                                endSelectD.text(firstDate);
                                startSelectD.text(date._i);
                            } else if (firstYear == date._i.substring(0, 4)) {//年份相等
                                if (firstMonth > date._i.substring(5, 7)) {//判断月份
                                    endSelectD.text(firstDate);
                                    startSelectD.text(date._i);
                                }else if(firstMonth < date._i.substring(5, 7)){
                                    endSelectD.text(date._i);
                                    startSelectD.text(firstDate);
                                }else if(firstMonth == date._i.substring(5, 7)){//月份相等
                                    if(firstDay > date._i.substring(8, 10)){
                                        endSelectD.text(firstDate);
                                    startSelectD.text(date._i);
                                    }else if(firstDay < date._i.substring(8, 10)){
                                         endSelectD.text(date._i);
                                    startSelectD.text(firstDate);
                                    }else{
                                         endSelectD.text(date._i);
        ...
        weekSelected: function (mom) {
            ...
            var end = null;
            var t = Kalendae.moment().subtract({d: 0});
            var e = moment(x).endOf('week').subtract('day', 0);
            var tr = t._d.getFullYear() > e._d.getFullYear() || (t._d.getFullYear() == e._d.getFullYear() && t._d.getMonth() > e._d.getMonth()) || (t._d.getFullYear() == e._d.getFullYear() && t._d.getMonth() == e._d.getMonth() && t._d.getDate() > e._d.getDate());
            var tr2 = t._d.getFullYear() > start._d.getFullYear() || (t._d.getFullYear() == start._d.getFullYear() && t._d.getMonth() > start._d.getMonth()) || (t._d.getFullYear() == start._d.getFullYear() && t._d.getMonth() == start._d.getMonth() && t._d.getDate() > start._d.getDate());

            if (tr) {
                end = moment(x).endOf('week').subtract('day', 0);
                this._sel = [start, end];
                var st = start._d;
                var en = end._d;
                startSelectD.text(st.getFullYear() + "-" + (parseInt(st.getMonth()) + 1) + "-" + st.getDate());
                endSelectD.text(en.getFullYear() + "-" + (parseInt(en.getMonth()) + 1) + "-" + en.getDate());
                this.publish('change', this, [mom.day()]);
                this.draw();
            }
            if (!tr && tr2) {
                end = Kalendae.moment().subtract({d: 1});
                this._sel = [start, end];
                var st = start._d;
                var en = end._d;
                startSelectD.text(st.getFullYear() + "-" + (parseInt(st.getMonth()) + 1) + "-" + st.getDate());
        ...
        monthSelected: function (mom) {
            var startSelectD = angular.element(document.querySelector('#startSelect'));
            var endSelectD = angular.element(document.querySelector('#endSelect'));
            var x = mom.toDate();
            var start = moment(x).startOf('month');
        ...
        //显示选择的日期
        makeSelectedStartDateVisible: function (date) {
        ...
        draw: function draw(n) {
            ...
            do {
                ...
                var da;
                if (n) {
                    mo = moment(n).startOf('day').hours(12);
                    da = moment(mo).date(1);
                }
                do {
                    if (opts.mode == 'week') {
                    ...
                    if (opts.mode == 'month' && n) {
                        if (((j % 7) === 0) && (j !== 0)) {
                            w++;
                        }
                        if (da.month() != mo.month()) {
                            $span = cal.days[w].childNodes[j % 7];
                        }
                    }
                    klass = [];
                    ...
                    if (s)
                        klass.push(({
                            ...
                            '2': classes.daySelectedFirst,
                            '3': classes.daySelectedLast,
                            'true': classes.daySelected //单选
                        })[s]);

                    var v1 = new Date(day);
                    var v2 = new Date(today);
                    var isTrue = v1.getFullYear() > v2.getFullYear() || (v1.getFullYear() == v2.getFullYear() && v1.getMonth() > v2.getMonth()) || (v1.getFullYear() == v2.getFullYear() && v1.getMonth() == v2.getMonth() && v1.getDate() >= v2.getDate());

                    if (day.month() != month.month() || isTrue) {
...
//				if (!(this.blackout(day) || this.direction(day) || (day.month() != month.month() && opts.dayOutOfMonthClickable === false))|| s>0) klass.push(classes.dayActive);
                    if ((!(this.blackout(day) || this.direction(day)) || s > 0) && day.month() == month.month())
...
// ELEMENT FUNCTIONS
        ...
        make: function (tagName, attributes, attach) {
            angular.element(document.querySelector('.kalendae')).remove();
...
// momentjs.com
    ...
    (function (undefined) {
        ...
        Language.prototype = {
            ...
            _weekdays: "周天_周一_周二_周三_周四_周五_周六".split("_"),
            weekdays: function (m) {
